...
class AstLowerer {
    ...
    private fun lowerStmt(stmt: Stmt): Unit = when (stmt) {
        ...
        is Stmt.ImportStmt -> {
            when (stmt.namespace.lexeme) {
                "math" -> {
                    // import math: load each math function as a bound method into globals
                    val mathReg = freshReg()
                    emit(IrInstr.LoadGlobal(mathReg, "math"))
                    val mathMethods = listOf("abs", "min", "max", "pow", "floor", "ceil", "round")
                    for (method in mathMethods) {
                        val methodReg = freshReg()
                        emit(IrInstr.GetField(methodReg, mathReg, method))
                        emit(IrInstr.StoreGlobal(method, methodReg))
                    }
                }
                "random" -> {
                    // import random: load each random function as a bound method into globals
                    val randomReg = freshReg()
                    emit(IrInstr.LoadGlobal(randomReg, "random"))
                    val randomMethods = listOf("random")
                    for (method in randomMethods) {
                        val methodReg = freshReg()
                        emit(IrInstr.GetField(methodReg, randomReg, method))
                        emit(IrInstr.StoreGlobal(method, methodReg))
                    }
                }
                "io" -> {
                    // import io: load each io function as a bound method into globals
                    val ioReg = freshReg()
                    emit(IrInstr.LoadGlobal(ioReg, "io"))
                    val ioMethods = listOf("print", "println")
                    for (method in ioMethods) {
                        val methodReg = freshReg()
                        emit(IrInstr.GetField(methodReg, ioReg, method))
                        emit(IrInstr.StoreGlobal(method, methodReg))
                    }
                }
                "time" -> {
                    // import time: load each time function as a bound method into globals
                    val timeReg = freshReg()
                    emit(IrInstr.LoadGlobal(timeReg, "time"))
                    val timeMethods = listOf("now", "sleep")
                    for (method in timeMethods) {
                        val methodReg = freshReg()
                        emit(IrInstr.GetField(methodReg, timeReg, method))
                        emit(IrInstr.StoreGlobal(method, methodReg))
                    }
                }
                else -> {
    ...
    private fun lowerTryCatch(stmt: Stmt.TryCatchStmt) {
        ...
        // Set up catch variable register first
        val catchReg = freshReg()
        locals[stmt.catchVar.lexeme] = catchReg

        emit(IrInstr.TryStart(catchLabel, catchReg))
        lowerBlock(stmt.tryBlock)
        emit(IrInstr.TryEnd)
        emit(IrInstr.Jump(endLabel))
        emit(IrInstr.Label(catchLabel))

        // Load pending exception into catch register
