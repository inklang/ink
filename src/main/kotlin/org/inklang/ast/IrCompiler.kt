    ...
    fun compile(result: AstLowerer.LoweredResult): Chunk {
        ...
        for (instr in result.instrs) {
            when (instr) {
                ...
                is IrInstr.TryStart -> chunk.write(OpCode.TRY_START, dst = instr.catchDst, imm = labelOffsets[instr.catchLabel.id]!!)
                is IrInstr.TryEnd -> chunk.write(OpCode.TRY_END)
                is IrInstr.Throw -> chunk.write(OpCode.THROW, src1 = instr.src)
                is IrInstr.LoadPendingException -> chunk.write(OpCode.LOAD_PENDING_EXCEPTION, dst = instr.dst)
            }
        }
