...
class VM {
    ...
    private val ioInstance = Builtins.newIo()
    private val timeInstance = Builtins.newTime()
    ...
    val globals = mutableMapOf<String, Value>(
        ...
        "Duration" to Value.Class(Builtins.DurationClass),
    ...
    data class CallFrame(
        ...
        var pendingException: Value? = null,  // Exception waiting to be caught in this frame
        var catchDst: Int = -1  // Destination register for catch variable, -1 if not set
    ) {
    ...
    fun execute(chunk: Chunk) {
        ...
        while (frames.isNotEmpty()) {
            ...
            when (opcode) {
                ...
                OpCode.TRY_START -> {
                    // dst = catch register, imm = catch label
                    frame.pendingException = null
                    frame.catchDst = dst
                }
                OpCode.TRY_END -> {
                    // Normal exit from try block - clear the try context
                    frame.catchDst = -1
                }
                OpCode.THROW -> {
                    val exception = frame.regs[src1] ?: error("Cannot throw null")
                    // Unwind frames looking for a try region
                    var foundTry = false
                    var currentFrameIdx = frames.size - 1
                    while (currentFrameIdx >= 0 && !foundTry) {
                        val f = frames[currentFrameIdx]
                        if (f.catchDst >= 0) {
                            // Found a frame with an active try - store exception and jump to catch
                            f.pendingException = exception
                            f.ip = imm  // Jump to catch label
                            foundTry = true
                        } else {
                    ...
                    if (!foundTry) {
                        // No catch found - propagate as uncaught exception
                        error("Uncaught exception: ${exception}")
                    }
                }
                OpCode.LOAD_PENDING_EXCEPTION -> {
                    frame.regs[dst] = frame.pendingException
                        ?: error("No pending exception to load")
                    frame.pendingException = null  // Clear after loading
                    frame.catchDst = -1  // No longer in a try-catch
                }
            }
        }
    }
