...
sealed class IrInstr {
    ...
    data class TryStart(val catchLabel: IrLabel, val catchDst: Int) : IrInstr()    // marks start of try region, catchDst is where to store exception
    object TryEnd : IrInstr()                                    // marks end of try region
    data class Throw(val src: Int) : IrInstr()                   // throws exception in src
    data class LoadPendingException(val dst: Int) : IrInstr()   // loads pending exception into dst
}
