...
enum class OpCode(val code: Byte) {
    ...
    TRY_START(0x2A), // marks start of try region, dst = catch reg, imm = catch label index
    TRY_END(0x2B),   // marks end of try region
    THROW(0x2C),     // throws exception in src1
    LOAD_PENDING_EXCEPTION(0x2D), // dst = current frame's pending exception
}
