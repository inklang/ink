...
class Parser(private val tokens: List<Token>) {
    ...
    private fun parseTryCatch(): Stmt {
        ...
        consume(TokenType.KW_CATCH, "Expected 'catch'")
        val catchVar = consume(TokenType.IDENTIFIER, "Expected catch variable name")
        val catchBlock = parseBlock()
        return Stmt.TryCatchStmt(tryBlock, catchBlock, catchVar)
    }
    ...
    private fun parseStmt(): Stmt {
        return when {
            ...
            check(TokenType.KW_THROW) -> parseThrow()
            else -> {
                val expr = parseExpression(0)
                if (check(TokenType.SEMICOLON)) advance()
                Stmt.ExprStmt(expr)
